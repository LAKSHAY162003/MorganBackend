...
// List of all events in which user has registered and attended but not followed up
...
router.post("/markUserFollowedUp", async (req, res) => {
  try {
    ...
    if (!event.followedUp.includes(userId)) {
      ...
      return res.status(500).json({ message: "User is already marked as followed up for this event." });
    }
